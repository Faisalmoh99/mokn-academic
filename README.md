...
نظام يحاكي فريق المرشدين الأكاديميين البشريين عبر أربعة وكلاء ذكيين يعملون تفاعلياً واستباقياً لاتخاذ قرارات أكاديمية مدروسة.
...
| 🛡️ **Guardian** | الحارس الاستباقي — يعمل كطبقة موازية مستقلة، يفحص سجلات الطلاب تلقائياً لاكتشاف مخاطر الحرمان والتعثر قبل أن يطلب الطالب المساعدة. | — |
...
[🎥 شاهد فيديو الديمو (دقيقتين)](https://www.loom.com/share/be11cce89ee3402eb2efeb734c995d11)

### 5 سيناريوهات جاهزة للعرض
...
5. **الفحص الاستباقي (Guardian)** — نظام رصد مباشر (Live SSE) للطلاب المعرضين للخطر، يولد تنبيهات مخصصة بأسلوب إرشادي دافئ
...
المعمارية الحالية تعتمد **Trust Split** واضح: مسار التفاوض التفاعلي يبقى داخل LangGraph (Orchestrator + Planner + Legis)، بينما يعمل Guardian **خارج LangGraph** كـ **Parallel Proactive Layer** مستقلة — تقييم المخاطر يتم بقواعد Python حتمية، والـ LLM مخصص فقط لصياغة الرسائل الإرشادية.
...
│                                                      │
│  ┌────────────────────────────────────────────────┐  │
│  │         Parallel Proactive Layer              │  │
│  │  ┌──────────┐                                  │  │
│  │  │ Guardian │  →  Live Alerts (SSE) → Dashboard
│  │  └──────────┘                                  │  │
│  └────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────┘
```
...
│   ├── data/
│   │   ├── mock/               # 5 طلاب + 33 مادة
│   │   └── demo_sessions/      # 5 جلسات محفوظة (4 تفاوض + الفحص الاستباقي Guardian)
